...
@app.route("/media/<media_id>/<country_code>/stories")
def media_country_stories(media_id, country_code):
    if USE_MENTIONED:
        stories = db.mentionedCountryStories(country_code, media_id)
    else:
        stories = db.countryStories(country_code, media_id)
    return render_template("media-country-stories.html",
        media_info = mediacloud.api.mediaSource(int(media_id)),
        stories = stories,
        country_code = country_code,
        country_name = _country_name(country_code)
...
def formatted_json_filter(value):
    return json.dumps(value, sort_keys = True, indent = 4)

app.jinja_env.filters['formatted_json'] = formatted_json_filter
